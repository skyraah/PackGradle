# P1 契约补全 DTO 草案（P1-CONTRACT）

> **状态：草案（prototype）——供反应，未定稿。** 决议后归档为执行规格。
> 来源：决策图「PackGradle 重构全路线决策图（Phase 1-4）」票 #5；本文件以 throwaway 分支 `prototype/p1-contract-draft` 承载，不进 main。
>
...
语义（按 ADR-0002 决议 5 注释）：`PolicyDTO.Revision`（策略集模板版本）与 `RelationDTO.Revision`（关系级策略代次）语义独立、互不驱动；两个数字都不进入用户可见文案或界面。写路径在 P1-POLICY 编译器落地前只做结构校验；编译器落地后 `UpdateMappingPolicy` 增加编译校验（`err.mapping.compile_failed` / `err.mapping.collision`，§4 预留）。
...
- 与 `PrepareRelation` 的关系：本票**不扩展** `PrepareRelationInput`（继续收 raw path，按需在预检内登记端点）；独立登记服务供 `/sources`、`/runtimes` 端点管理页使用（决策 D5）。
...
`ResolvePlan` 从 draft plan 继承 exactness（不可变），与既有 digest 语义一起固化。`normalization_version` 与 `policy_digest` 的固化属 P1-PLAN 执行票（`policy_digest` 已存在于 DTO），不在本票范围。
...
- **D1 Features/Availability 挂载点**：内嵌 `WorkspaceDTO`（列表与详情同构）。推荐此方案——架构 §10.3 缓存键 `workspaceDetail` 已定为「工作区、features 与 action availability」一体，列表页同样需要 availability 渲染操作按钮。
- **D2 资源级 Changes 形态**：独立分页查询 `GetChanges`（快照对 + 筛选 + 游标），**推荐**；备选：内嵌进 `WorkspaceDTO` 一次拉全量（列表页被 3,000 资源拖垮，且无法按分类/路径筛选）。
- **D3 rebind 的 baseline 继承**：P1 恒 `reinitialize`（ApplyRebind 后进入 initialization_required），**推荐**——等价性证明需要完整扫描，P1 不承担；`inherit` 留 Phase 2。不推荐 P1 做试算扫描（把慢操作塞进预检）。
- **D4 端点用例服务归属**：`ProjectService` + `RuntimeService` 两服务（对齐架构 §4.2 目录），**推荐**；备选：合并单一 `EndpointService`（注册面少一个服务，但偏离架构目录，且 Project/Runtime 发现语义不同）。
- **D5 PrepareRelation 输入是否扩展为 endpoint_id**：本票不扩展（维持 raw path，独立登记服务幂等），**推荐**——P1 前端落地前无消费方，避免双通道输入语义漂移；前端接入时若需要再议。
