...
**A 口径结论**：收口自检九命令一次依序执行，逐命令退出码——`test`=0、`test:vet`=0、`test:race`=0、`acceptance:headless`=0、`acceptance:recovery`=0、`acceptance:recovery:restore`=0、`acceptance:download`=0、`acceptance:gc`=0、`acceptance:perf`=非零（唯一超标项 restore 冷链路的门槛评估失败传播；超标处置记录已随 `p3-perf-*-restore.json` 归档，分析见 §3.1）。L0 全绿（性能一项按规格完成超标处置记录——按验收规格 §7/P2 §3「超标处置沿 P2：记录原因与机器规格，环境异常注明后重测」处置；如执行会话判定须达标后收口，重测后更新本表）。
...
| 热扫描 | ≤2s | 0.45s | ✅ |
| 热命中率 | ≥95% | 100% | ✅ |
| 冷 apply（2400 op） | ≤30s | 18.8s | ✅ |
| Apply 峰值内存增量 | <256MiB | 17.0 MiB | ✅ |
| **restore 冷全链路（4800 op，双侧写回）** | **≤30s** | **64.9s（多轮 51.7–65.9s；staging=21.6–35.6s applying=14.1s verifying=13.1s）** | **⚠️ 超标处置** |
| **restore 峰值内存增量** | **<256MiB** | **4.3–8.4 MiB** | ✅ |
| **GC** | **≤30s** | **8.1s** | ✅ |
| download 相位（staging 下载墙钟） | 只记录 | 0（perf 夹具无 redownload 行，零网络口径） | 记录 |

### 3.1 restore 冷超门槛处置记录（按 §7/P2 §3）

- **现象**：多轮实测 51.7s / 65.9s / 65.7s / 64.9s（staging 相位波动最大：11.5s→35.6s→21.6s→35.6s），均超 30s 门槛；内存增量 4.3–12.3 MiB 远低门槛。
