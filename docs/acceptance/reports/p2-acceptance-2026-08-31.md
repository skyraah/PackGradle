...
- [x] 强杀后重启呈现——应用冷启动同步执行恢复管线，列表行徽标「需要恢复」（关系健康列）+ 行内「处理恢复」主上下文动作，状态全部来自查询 API（重启两次呈现一致，重复恢复幂等，无重复补偿/二次破坏）。**偏差注记（规格 §1.2「恢复横幅/短状态按 UX §5.3 呈现」）**：未建独立横幅组件——恢复状态实际由列表行徽标、plans 页门控区（apply 不可用显后端原因码文案，§8.1）与恢复详情页三处承载，横幅的可观测职责已被覆盖，「状态以查询 API 为准」口径不变
