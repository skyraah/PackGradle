...
> **⚠️ 视觉基准变更（2026-08-28）**：本节视觉方向已被 [`概念风格.pdf`](./概念风格.pdf) 取代——内容区平面底 #1B212D（渐变方案因视觉效果不佳弃用）、壳层（标题栏 + icon rail）#121720、祖母绿 #4ADE80 强调（不再沿用青色）。不再以 PCL2 风格为视觉指导。本节其余密度/窗口规格（§11.2、§11.3）继续有效。可交互原型见 [`workspace-ux-prototype.html`](./workspace-ux-prototype.html)。
