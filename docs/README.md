...
- 错误只以 `err.*` 错误码从 Go 端传递，文案统一在 `frontend/src/locales/zh-CN.json`（497 个键，其中 101 个 `err.*`、12 个 `diag.*`；T16 旧栈退场后清除 360 余个死键）；code→locale 一致性由 conformance test 自动比对（`internal/errs/locale_conformance_test.go`，缺键/死键即红，随 `task test` 运行）；逐资源诊断（`diag.*`）随快照/计划透出，不作为调用级错误。
...
- 新架构（store/sqlite + transport + pgheadless）已入主干：headless 入口 `cmd/pgheadless`（PrepareRelation → CreateRelation → StartScan → PrepareSync → GetPlan，可重复执行，已有 Relation 自动复用；`-resolve` 补全 ResolvePlan → GetPlan 供 A 口径链路验收）；验收 L0 命令已挂 Taskfile：`task test` / `task test:vet` / `task test:race`（`-race` 需 mingw-w64 gcc 提供 CGO 工具链）/ `task acceptance:headless`（小规模 fixture + 全新数据目录全链路两遍，票 #25）。
- 性能基线（验收规格 §2，票 #24）：`task acceptance:perf` 一键执行——`cmd/pgfixture` 生成 3,000 资源确定性 fixture（固定 seed、产物入 build/perf/ 不入 git）→ pgheadless `-metrics` 冷/热两轮（分项 JSON：扫描四相耗时 + hash cache 命中 delta，schema p1-perf-run/1）→ `pgfixture -eval` 门槛评估（冷 ≤10s / 热 ≤2s / 热命中率 ≥95%，超标退出码 1）。首测记录见 `docs/acceptance/records/`。
- A 口径验收报告（票 #25）：[acceptance/reports/p1-acceptance-2026-08-30.md](./acceptance/reports/p1-acceptance-2026-08-30.md)——L0 五命令全绿（含 `-race`）、性能三项达标、P0-1/P0-2 前端部分显式 deferred。
