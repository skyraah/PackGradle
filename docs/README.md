...
- 新架构（store/sqlite + transport + pgheadless）已入主干：headless 入口 `cmd/pgheadless`（Register → Scan → Snapshot → PrepareSync → ResolvePlan → GetPlan 可重复执行）；验收 L0 命令已挂 Taskfile：`task test` / `task test:vet` / `task test:race`（`-race` 需 mingw-w64 gcc 提供 CGO 工具链）。
