...
- 后端向 Wails 注册 6 个服务：`EnvService`（7 个方法）、`PackwizService`（8 个方法）、`PrismService`（26 个方法）+ 新架构 `SyncService`（14 个方法）、`ProjectService`（4 个方法）、`RuntimeService`（4 个方法），共 63 个可调用方法。
- 前后端经 `src/api` 门面按契约调用真实绑定；mock 层（`src/mocks` 内存库）保留，可经设置页开关 / 顶栏 MOCK 徽标一键切换；文件/目录选择经 `utils/dialogs.ts`（`Dialogs.OpenFile`）。
- 错误只以 `err.*` 错误码从 Go 端传递，文案统一在 `frontend/src/locales/zh-CN.json`（620 个键，其中 83 个 `err.*`、12 个 `diag.*`）；逐资源诊断（`diag.*`）随快照/计划透出，不作为调用级错误。
- 新栈前端页：`/workspaces`（工作区列表，契约 04 事件/缓存骨架：单订阅点 + 受控重查管线 + bootstrap 恢复）、`/sources`（项目源）与 `/runtimes`（运行实例）已落 shadcn-vue；changes/mappings/plans/rebind 页随后续票施工。
