...
type ModsWatchEvent struct {
	Project string   `json:"project"`         // 项目名
	Side    string   `json:"side"`            // 触发端："project"（项目 mods）/ "instance"（实例 .index）/ "both"
	Diff    MetaDiff `json:"diff"`            // 双端数据比对结果（Error 非空时为零值）
	Error   string   `json:"error,omitempty"` // 比对失败原因（errs JSON 文本）
}
