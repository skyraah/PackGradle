...
// /runtimes：Prism 运行实例端点管理（UX 原型 E-02 画板，票 #109；契约 03 §2.5）。
// 只管理运行实例的发现、登记与绑定健康，没有跨端写入动作。七列表格（运行实例/
// 启动器/根路径/绑定健康/关联工作区/最近检查/更多），行点击开 400px 详情抽屉
// （字段/诊断/技术信息三分区）；登记走对话框（「登记并选中」）；移除登记走危险
// 确认弹窗。健康结果读写 stores/endpoints 会话缓存（票 #109）：进页自动补查缓存
// 缺失的端点，切页往返已查健康不丢；关联工作区计数取 syncCache 工作区投影（按
// 登记根路径匹配，沿新建向导先例）。发现（Prism 实例自动发现）保留为表格下方辅助区。
...
import type { EndpointDTO, RuntimeCandidateDTO } from '../api'
    ...
    <div class="mx-auto flex w-full max-w-6xl flex-col gap-4 p-4 text-foreground">
        ...
        <div class="flex items-start justify-between gap-4">
            ...
            <Button size="sm" @click="regOpen = true">
                <PlusIcon class="size-3.5" />{{ t('runtimes.registerAction') }}
            </Button>
        </div>

        <!-- 已登记运行实例：七列表格，行点击开详情抽屉 -->
        <Card>
            <CardContent class="py-2">
                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableHead>{{ t('runtimes.colName') }}</TableHead>
                            <TableHead>{{ t('runtimes.colAdapter') }}</TableHead>
                            <TableHead>{{ t('endpoints.colPath') }}</TableHead>
                            <TableHead>{{ t('runtimes.colHealth') }}</TableHead>
                    ...
                    <TableBody>
                        ...
                        <TableRow v-else-if="!registered.length">
                            <TableCell :colspan="7">
                                <p class="text-muted-foreground py-8 text-center text-sm">{{ t('runtimes.registeredEmpty') }}</p>
                            </TableCell>
                        </TableRow>
                        <TableRow
        ...
        <Dialog v-model:open="regOpen">
            <DialogContent class="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle>{{ t('endpoints.regDialogTitle', [t('runtimes.title')]) }}</DialogTitle>
                    <DialogDescription>{{ t('endpoints.regDialogNote') }}</DialogDescription>
                </DialogHeader>
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-medium" for="runtimes-reg-path">{{ t('endpoints.rootPathField') }}</label>
                    <div class="flex gap-2">
                        <Input
                            id="runtimes-reg-path"
                            v-model="regPath"
                ...
                <div v-if="drawerEp" class="min-h-0 flex-1 overflow-y-auto p-4">
                    <div class="flex flex-col gap-1.5 text-sm">
                        ...
                        <div class="flex items-center justify-between gap-3">
                            <span class="text-muted-foreground text-xs">{{ t('runtimes.colAdapter') }}</span>
                            <Badge variant="outline">{{ drawerEp.adapter }}</Badge>
                        </div>
                        <div class="flex items-center justify-between gap-3">
                            <span class="text-muted-foreground text-xs">{{ t('runtimes.colHealth') }}</span>
